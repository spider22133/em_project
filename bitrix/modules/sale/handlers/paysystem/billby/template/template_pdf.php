...
if (count($params['BASKET_ITEMS']) > 0)
{
	...
	if ($params['BILLBY_TOTAL_SHOW'] == 'Y')
	{
		if ($sum < $params['SUM'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBY_SUBTOTAL'));
		...
		if (!$params['TAXES'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBY_TOTAL_VAT_RATE'));
			$arCells[$n][$arColumnKeys[$columnCount-1]] = CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILLBY_TOTAL_VAT_RATE_NO'));
		}
...
if ($params['BILLBY_TOTAL_SHOW'] == 'Y')
{
	...
	{
		$pdf->Write($lineHeight, CSalePdf::prepareToPdf(
			Number2Word_Rus($params['SUM'], "Y", $params['CURRENCY']))
		);
	}
	else
	{
