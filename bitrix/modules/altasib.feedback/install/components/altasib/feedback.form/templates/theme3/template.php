...
<?endif?>
<div class="alx_feed_back_form alx_feed_back_theme3" id="alx_feed_back_theme3_<?=$ALX?>">
		...
		<div class="alx_feed_back_form_error_block">
			...
			<tr>
				...
				<td class="alx_feed_back_form_error_td_list">
					...
					<div class="alx_feed_back_form_error_list"><?=GetMessage("ALX_TP_CORRECT_DATE");?><?
					if(!empty($arResult["FORM_ERRORS"]["ELEMENT_ADD"]["ELEMENT"])
						|| !empty($arResult["FORM_ERRORS"]["MESS_ADD"]["MESSAGE"])):?>
						<ul><li><span>-</span> <?=$arResult["FORM_ERRORS"]["ELEMENT_ADD"]["ELEMENT"]?></li></ul><?
					endif;?>
					</div>
				</td>
			</tr>
			...
			<div class="alx_feed_back_form_item_pole">
				...
				<div class="alx_feed_back_form_inputtext_bg">
					...
					<select id="type_question_<?=$ALX?>" name="type_question_<?=$ALX?>" onchange="ALX_SetNameQuestion(this,'<?=$ALX?>');">
						<option value=""><?=GetMessage("ALX_CATEGORY_NO");?></option>
...
?>
<?	if(
		(is_array($arParams["PROPERTY_FIELDS"]) && in_array("FEEDBACK_TEXT", $arParams["PROPERTY_FIELDS"]))
		|| (
...
<?	foreach($arResult["FIELDS"] as $key=>$arField):?>
		<div class="alx_feed_back_form_item_pole <?if(in_array($arField['CODE'], $errorField, true)):?> alx_feed_back_form_error_pole<?endif?>">
...
<?		endif?>
<?			if(in_array($arField["CODE"], $errorField, true)):?>
				<div class="alx_feed_back_form_error_text"><?=$arResult["FORM_ERRORS"]["EMPTY_FIELD"][$arField["CODE"]]?></div>
<?			endif?>
		</div>
<?
...
<?				$capCode = $GLOBALS["APPLICATION"]->CaptchaGetCode();?>
				...
				if($arParams["CHANGE_CAPTCHA"] == "Y"):
					?><a href="#" onclick="ALX_ChangeCaptcha('<?=$ALX?>');return false;"><?=GetMessage("ALX_TP_RELOADIMG")?></a><?
				else:
					?><a href="#" onclick="capCode='<?=htmlspecialcharsEx($capCode)?>';ALX_ReloadCaptcha(capCode,'<?=$ALX?>');return false;"><?=GetMessage("ALX_TP_RELOADIMG")?></a><?
				endif;
				?></small></div>
...
<?				if(in_array("ALX_CP_WRONG_CAPTCHA", $errorField, true)):?>
						<div class="alx_feed_back_form_error_text"><?=GetMessage("ALX_CP_WRONG_CAPTCHA");?></div>
<?				endif?>
			</div>
<?		else:?>
...
<?				if(in_array("ALX_CP_WRONG_CAPTCHA", $errorField, true)):?>
					<div class="alx_feed_back_form_error_text"><?=$arResult["FORM_ERRORS"]["CAPTCHA_WORD"]["ALX_CP_WRONG_CAPTCHA"]?></div>
<?				endif?>
				</div>
...
$(document).ready(function()
{
	var file_w_<?=$ALX?> = parseInt($("#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles").width()/5);
...
<?	for($i=1;$i<=$k;$i++):?>
	$("#alx_feed_back_theme3_<?=$ALX?> #alx_feed_back_form_file_input_add<?=$i?>")
		...
		.change(function(){
			...
			$("#alx_feed_back_theme3_<?=$ALX?> #alx_feed_back_form_filename<?=$i?>").text(myStr_<?=$ALX?>_<?=$i?>);
		});
<?	endfor?>
});
-->
...
<style type="text/css">
#alx_feed_back_theme3_<?=$ALX?>
{
	width:<?=str_replace(" ", "", $arParams["WIDTH_FORM"])?> !important;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_error_block
{
...
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_error_list
{
	color:<?=str_replace(" ", "", $arParams["COLOR_ERROR"])?>;
	font-size:<?=str_replace(" ", "", $arParams["SIZE_INPUT"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_title_error
{
	color:<?=str_replace(" ", "", $arParams["COLOR_ERROR_TITLE"])?>;
	font-size:<?=str_replace(" ", "", $arParams["SIZE_NAME"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_mess_ok
{
	color:<?=str_replace(" ", "", $arParams["COLOR_MESS_OK"])?>;
	font-size:<?=str_replace(" ", "", $arParams["SIZE_NAME"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_name
{
	font-size:<?=str_replace(" ", "", $arParams["SIZE_NAME"])?>;
	color:<?=str_replace(" ", "", $arParams["COLOR_NAME"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_hint
{
	font-size:<?=str_replace(" ", "", $arParams["SIZE_HINT"])?>;
	color:<?=str_replace(" ", "", $arParams["COLOR_HINT"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_item_pole
{
	font-size:<?=str_replace(" ", "", $arParams["SIZE_NAME"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_error_pole,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_inputtext_bg,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_inputtext_bg_file,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_submit_block,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_file_button .alx_feed_back_form_file_button_bg
{
	-moz-border-radius:<?=str_replace(" ", "", $arParams["BORDER_RADIUS"])?>;
	-webkit-border-radius:<?=str_replace(" ", "", $arParams["BORDER_RADIUS"])?>;
	-khtml-border-radius:<?=str_replace(" ", "", $arParams["BORDER_RADIUS"])?>;
...
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_inputtext_bg input,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_inputtext_bg textarea,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_inputtext_bg select,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_filename,
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_file_button_bg
{
	font-size:<?=str_replace(" ", "", $arParams["SIZE_INPUT"])?>;
	color:<?=str_replace(" ", "", $arParams["COLOR_INPUT"])?>;
	font-family:tahoma;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_file_input_add
{
	font-size:<?=str_replace(" ", "", $arParams["SIZE_INPUT"])?>;
	color:<?=str_replace(" ", "", $arParams["COLOR_INPUT"])?>;
}
#alx_feed_back_theme3_<?=$ALX?> .alx_feed_back_form_feedback_poles .alx_feed_back_form_required_text
{
	color:red;
}
</style>
