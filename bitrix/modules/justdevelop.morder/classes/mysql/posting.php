<?
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/justdevelop.morder/classes/general/posting.php");

class morderPosting extends morderPostingGeneral
{
		...
		if (is_array($arFilter))
		{
			foreach($arFilter as $key=>$val)
			{
				...
				case "FROM":
					...
					break;
				case "TO":
					$r = GetFilterQuery("PE.PHONE", $val, "Y", array("@","_","."));
					if(strlen($r) > 0)
						$arSqlSearch[] = "EXISTS (SELECT * FROM JUSTDEVELOP_posting_sms PE WHERE PE.POSTING_ID=P.ID AND PE.STATUS='N' AND ".$r.")";
					break;
				case "BODY_TYPE":
				...
				case "RUB_ID":
					if(is_array($val) && count($val) > 0)
					{
						...
						if(count($rub_id))
							$arSqlSearch[] = "EXISTS (SELECT * from JUSTDEVELOP_posting_rubric PR WHERE PR.POSTING_ID = P.ID AND PR.LIST_RUBRIC_ID in (".implode(", ", $rub_id)."))";
					}
					break;
				case "BODY":
