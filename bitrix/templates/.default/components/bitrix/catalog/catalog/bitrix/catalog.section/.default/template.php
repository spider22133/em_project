...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div itemscope itemtype="http://schema.org/Product" class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">

        <a type="button" class="btn btn-wish js-wishlist"  data-wishid="<?=$arItem['ID']?>" title="Добавить в список желаний" data-wishiblock="<?=$arResult["IBLOCK_ID"]?>"></a>
        <?$APPLICATION->IncludeComponent(
            ...
            Array(
                ...
                "PERSONAL_DATA" => "N",
                "PERSON_TYPE" => "1",
                "PERSON_TYPE_PROPS" => array("1", "3"),
                "PRICE_CODE" => "Розничная цена",
                "PRODUCT_ID" => $arItem,
                "TITLE_POPUP" => "Быстрый заказ",
                "USE_CAPTCHA" => "N",
                "USE_COMMENT" => "N",
                "USE_COUNT" => "Y",
                "USE_USER" => "N"
        ...
        if ($arItem['PROPERTIES']['PROVERENO']['VALUE'] == 'да'){
        ?><div class="ribbon-wrap right-edge fork lblue"><span>ПРОВЕРЕНО</span></div>
        <? } ?>
	...
	{ $VALUE = '';
        $res = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $arItem['ID'], "sort", "asc", array("CODE" => "MORE_PHOTO"));
        if ($ob = $res->GetNext())
        {
            $VALUE = $ob['VALUE'];
        }
	...
	?><div class="bx_catalog_item_title"><a itemprop="name" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a></div>
	<div class="bx_catalog_item_price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
        <div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><strong itemprop="price"><?
	if (!empty($minPrice))
	{
	...
	unset($minPrice);
	?></strong><strong itemprop="priceCurrency"> грн.</strong></div>
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		if ($arItem['CAN_BUY'])
		{
			...
			{
			?>
		<div class="bx_catalog_item_controls_blockone col-xs-6 pull-left" style="padding-left: 0px;padding-right: 0px;text-align: left;"><div style="display: inline-block;position: relative;">
			...
			?>
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo col-xs-6 pull-right" style="padding-right: 0px;text-align: right;" role="group" aria-label="...">



			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="btn btn-cart" href="javascript:void(0)" role="button" rel="nofollow" ><?
...
?><div style="clear: both;"></div>
    ...
    <? $this->SetViewTarget('desc_r');
    if (is_object($arResult['NAV_RESULT']) && $arResult['NAV_RESULT']->PAGEN == 1): ?>
    <div class="col-xs-12">
        <div class="desc_r" style="text-align: justify;">
