...
var VOptionsAddGroupParams = function(id)
{
	...
	// 	_name = name + '[' + group_cnt + '][' + i + ']'
	// 	_id = id + '-' + group_cnt + '-' + i;
	// 	html += VOptionsAddGroupParam(param, _name, _id, id_orig, i);
	// });
...
var VOptionsAddGroupParam = function(param, name, id, group_arr, group_name)
{
	...
	{
		html = '<tr>';
		html += '<td width="40%" class="adm-detail-content-cell-l adm-detail-valign-top">';
		...
		switch(param.TYPE)
		{
			...
			case 'TEXTAREA':
				...
				if(is('param.CHOISE') && is('param.VALUES'))
				{
					...
					html += '<label class="voptions-textarea-choise-checkbox-label" for="' + id + '_choise_checkbox"><?=GetMessage('TEXTAREA_SHOW_CHOISE')?></label>';
