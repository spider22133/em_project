...
<?if ($arResult["HIDE_BUTTON"] != "Y"):?>
	<a class="btn btn-primary js-popup-btn-form" href="#popup-window<?=$arResult["UNIQUE_CODE"]?>"><?=GetMessage("SALE_BTN")?></a>
<?endif;?>
		...
		<form action="<?=$APPLICATION->GetCurPage()?>" name="popup-form<?=$arResult["UNIQUE_CODE"]?>" id="popup-form<?=$arResult["UNIQUE_CODE"]?>" method="post" role="form">
			...
			<div class="row">
				<?if (!empty($arResult["ELEMENTS"])):?>
					<div class="col-sm-12 popup-title"><?=$arResult["ELEMENTS"]["NAME"]?></div>
			...
			<?if ($arResult["HIDE_BUTTON"] != "Y"):?>
				<div class="row">
					<div class="col-sm-12 popup-btn">
						<input type="submit" name="popup-order-confirm" class="js-popup-order-confirm btn btn-primary" data-id="<?=$arResult["UNIQUE_CODE"]?>" value="<?=GetMessage('SALE_BTN_ORDER')?>">
					</div>
				</div>
			<?endif;?>
		</form>
