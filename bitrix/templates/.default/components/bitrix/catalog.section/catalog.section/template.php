...
<div class="title-wrapper">
	<h2 class="title">Сейчас в тренде
			<?
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		...
		<a type="button" class="btn btn-wish js-wishlist" title="Добавить в список желаний" data-wishid="<?=$arItem['ID']?>" data-wishiblock="<?=$arResult["IBLOCK_ID"]?>"></a>
...
<?
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		if ($arItem['CAN_BUY'])
		{
			...
			{
			?>
		<div class="bx_catalog_item_controls_blockone add_form col-xs-6 pull-left" style="padding-left: 0px;padding-right: 0px;text-align: left;"><div style="display: inline-block;position: relative;">
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	else
	{
		...
		<div id="<? echo $arItemIDs['NOT_AVAILABLE_MESS']; ?>" class="bx_catalog_item_controls_blockone" style="display: <? echo ($canBuy ? 'none' : ''); ?>;"><span class="bx_notavailable"><?
			echo ('' != $arParams['MESS_NOT_AVAILABLE'] ? $arParams['MESS_NOT_AVAILABLE'] : GetMessage('CT_BCS_TPL_MESS_PRODUCT_NOT_AVAILABLE'));
		?></span></div>
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="btn-group pull-right" style="display: <? echo ($canBuy ? '' : 'none'); ?>;" role="group" aria-label="...">
			      <a id="<? echo $arItemIDs['BUY_LINK']; ?>"  href="javascript:void(0)" class="btn btn-cart" role="button" rel="nofollow"></a>
		</div>
		<?
	if ($arParams['DISPLAY_COMPARE'])
	{
