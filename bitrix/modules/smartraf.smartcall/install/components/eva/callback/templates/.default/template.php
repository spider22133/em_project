  ...
  <span class="wr_close"><img src="<?=$templateFolder?>/images/cls_btn.png" /></span>
   ...
   <div class="frm_place">
		<form action="<?=$componentPath?>/script/senddata.php" method="POST" id="call_ord">
			<?=bitrix_sessid_post()?>
			<div>
				<input type="text" name="v_name" id="v_name" placeholder="<?=GetMessage("MFT_NAME")?>" value="" maxlength="30"<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])):?> required<?endif?> />
				<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("NAME", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?endif?>
			</div>
			<div>
				<input type="text" name="v_phone" id="v_phone" placeholder="<?=GetMessage("MFT_PHONE")?>" value="" maxlength="20"<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("PHONE", $arParams["REQUIRED_FIELDS"])):?> required<?endif?> />
				<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("PHONE", $arParams["REQUIRED_FIELDS"])):?><span class="mf-req">*</span><?endif?>
			</div>
			<div>
				<input type="text" name="v_time" id="v_time" placeholder="<?=GetMessage("MFT_TIME")?>" value="" maxlength="10"<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("TIMETOCALL", $arParams["REQUIRED_FIELDS"])):?> required<?endif?> />
			...
			<div>
				<textarea name="v_mess" id="v_mess" maxlength="150"<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("MESSAGE", $arParams["REQUIRED_FIELDS"])):?> required<?endif?>><?=GetMessage("MFT_MESSAGE")?></textarea>
