	...
	if($_POST['ALX_COMMON_CRM']=='Y')
	{
		...
		else
		{
			if(!trim($_POST['ALX_FEEDBACK_HASH'])
				...
				&& ($siteK == $_POST['ALX_RECAPTCHA_SITE_KEY'] && $secretK == $_POST['ALX_RECAPTCHA_SECRET_KEY'])
			)
					CAdminMessage::ShowNote(GetMessage('ALX_FEEDBACK_LOGIN_PASS_ERROR'));
		}
	}
	else
	{
		...
		while($arSite = $dbSites->GetNext())
		{
			...
			else
			{
				if(!trim($_POST['ALX_FEEDBACK_HASH_'.$arSite['LID']])
					&& !trim($_POST['ALX_RECAPTCHA_SITE_KEY_'.$arSite['LID']]) && !trim($_POST['ALX_RECAPTCHA_SECRET_KEY_'.$arSite['LID']])
				)
					CAdminMessage::ShowNote(GetMessage('ALX_FEEDBACK_LOGIN_PASS_ERROR_FOR').$arSite['LID']);
			}
		}
		...
		<td colspan="2">
			<div class="notes">
				<table cellspacing="0" cellpadding="0" border="0" class="notes" align="center">
					...
					<tr>
						<td class="left"><div class="empty"></div></td>
						<td class="content"><?echo GetMessage("ALTASIB_FEEDBACK_DESCR")?></td>
						<td class="right"><div class="empty"></div></td>
					</tr>
					<tr class="bottom">
						<td class="left"><div class="empty"></div></td>
